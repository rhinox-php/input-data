...
class InputData implements \ArrayAccess, \Countable, \IteratorAggregate, \JsonSerializable
{
     ...
     * @param mixed  $default The default value if the item doesn't exist
     */
    public function raw(?string $name = null, $default = null)
    {
        [$data, $name] = $this->extractDataKey($name, $this->_data);
    ...
    public static function jsonDecode(string $jsonString, bool $assoc = true, int $depth = 512, $flags = JSON_THROW_ON_ERROR): static
    {
        $json = json_decode($jsonString, $assoc, $depth, $flags);
    ...
    public static function tryJsonDecodeFile(string $filename, bool $assoc = true): static
    {
        ...
        } catch (FileReadException | \JsonException $exception) {
            return new static(null);
        }
    }
